# Contributing to Milpa Live

Thanks for your interest in contributing! Milpa Live is the render-target-agnostic
live component core of the Milpa framework — component contracts, mount/handle
lifecycle, data sources, and an event-driven interception seam. The same component
definition renders to web AND terminal; a component never emits HTML or ANSI
itself, a `ComponentRendererInterface` does. It builds on `milpa/core`: attributes,
interfaces, events, and the verification seam, with no product coupling.
...
- **Respect the tier boundary.** Milpa Live depends on `milpa/core`, never the
  reverse. Do not introduce a dependency on Doctrine, a concrete container, or any
  product/plugin code, and do not push live-component concerns down into the core.
- **Stay render-target-agnostic.** A component definition (`ComponentDefinitionInterface`)
  MUST NOT emit HTML, ANSI, or any other render-target-specific output — that is a
  `ComponentRendererInterface`'s job. Do not couple `src/Components` to a specific
...
The whole Milpa family (`milpa/core`, `milpa/http`, `milpa/tool-runtime`, `milpa/live`)
